...
14. [Strict quantized-cache profile and scoped runtime patch](14-strict-quant-cache-profile.md) - source-derived failure gate for full-pool FP16 cache expansion, independent pinned checkout, SHA-validated text+MTP profile, and read-only prepared/armed preflight. All checks so far are CPU-only; runtime parity and benchmarks are deferred.
